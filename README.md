...
- [gh-draft](gh-draft) - Create or update GitHub issues/PRs from markdown drafts with YAML frontmatter (labels, assignees, milestone, blocked-by deps; PR base/draft/reviewers; any repo via `repo:`/`-R`). Creating records the number back into the draft so re-runs update instead of duplicating; updates are add-only with a word-diff preview and confirmation, and refuse to touch other people's issues. Since a body is published verbatim and GitHub therefore skips the repository template, a draft missing the local template's sections is flagged before publishing (`--no-template-check` to skip). `gh-draft get <number> [dest]` downloads an issue/PR into a round-trippable draft — to an explicit file, a generated `<title-slug>.md` in a directory, stdout (`-`), or the clipboard (`-c`, body only).
- [gh-issue-get](gh-issue-get) - Fetch a GitHub issue body and copy it to the clipboard.
- [git-contributors](git-contributors) - Collect every contributor from the git history (default: all of HEAD, or any range/options `git log` understands), including `Co-authored-by` trailers, mailmap-canonicalized and deduplicated by email, and print per-contributor statistics as CSV on stdout; `-c` prints a ready-to-paste `Co-authored-by:` block for a squashed commit instead (`-x` leaves yourself out).
...
- [tmux-startup](tmux-startup) - Build tmux sessions, windows and panes from a JSON layout config (`~/.config/tmux-startup/config.json`), so nothing machine- or client-specific lives in the script: sessions name their windows, panes, layout, working directories and commands, `{hostname}` expands to the host, and extra tmux servers (own socket, own conf) are just another entry. Existing sessions are left alone, `--list` shows what is configured, `--dry-run` prints the tmux commands instead of running them, and `--example` prints a starter config. Machine setup (ssh keys, keyboard quirks, `loginctl enable-linger`) goes in executables under `hooks/pre/` and `hooks/post/` next to the config. Python 3, no dependencies.
